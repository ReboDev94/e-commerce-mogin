...
const { itemSelected, previousItem, nextItem, hasNext, hasPrevious, setItems } =
  useCarousel<IProductOrder>();
  ...
  <div class="col-span-12">
    <div v-if="itemSelected" class="h-52 max-h-52">
      <div class="relative h-36 px-3 py-2 grid place-content-center">
        <div
          v-if="hasPrevious"
          class="z-10 absolute top-1/2 -translate-y-1/2 h-9 w-9 grid place-content-center"
        >
          <button
            class="btn btn-circle btn-primary btn-xs btn-outline"
            @click="previousItem"
          >
        ...
        <div class="absolute inset-0 grid place-content-center">
          <img
            :src="itemSelected.img || 'https://placehold.co/300x400'"
            class="h-32 w-28 rounded-md"
            alt="drink"
          />
        </div>
        <div
          v-if="hasNext"
          class="z-10 absolute top-1/2 right-0 -translate-y-1/2 h-9 w-9 grid place-content-center"
        >
          <button
            class="btn btn-circle btn-primary btn-xs btn-outline"
            @click="nextItem"
